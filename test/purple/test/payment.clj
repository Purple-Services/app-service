...
(deftest confirm-stripe-req-merge-with
  ...
  (let [common-opts {:as :json :coerce :always}
    ...
    (is (= opts {:as :json :coerce :always :form-params {:foo "bar"} :headers {"baz" "qux"}}))
    (let [add-opts (->> opts
      ...
      (is (= add-opts {:as :json :coerce :always :form-params {:foo "bar" :quux "corge"} :headers {"baz" "qux" "grault" "garply"}})))
    ))
